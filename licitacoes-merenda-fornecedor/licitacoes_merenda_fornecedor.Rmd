...
## Quais são os fornecedores de merenda de cada município?
...
Como esperado, viu-se que as maiores cidades possuem mais fornecedores, que as demais, e mesmo entre as 10 maiores cidades (em população) a discrepância é evidente; Um exemplo disso é a cidade de João Pessoa, que sozinha possui mais fornecedores que todas as outras 9.
Quanto as demais cidades do estado, o número máximo

## Quantos fornecedores, em média, competem nas licitações?
```{r}
# FALTA organizar os dados
mean(licitacoes$nu_Propostas)
summary(licitacoes$nu_Propostas)

```
Observa-se que na imensa maioria das licitações a concorrência é entre 2 ou 3 candidatos, o que faz com que a média de propostas seja também, relativamente baixa.
...
ggplot(aes(x = nu_Propostas, y = vl_Licitacao), data = subset(licitacoes, vl_Licitacao < 5000000)) + geom_point(color = '#8C1717') + 
  ...
  # xlim(c(0, 22)) + ylim(c(0, 1000000))
```
O valor de correlação obtido é insuficiente para afirmarmos que há influência do valor da licitação sobre o número de concorrentes desta.



## Existem fornecedores que dominem a distribuição de merenda?
Observa-se que a imensa maioria dos fornecedores ganhou 5 licitações ou menos, no entanto, um pequeno número de fornecedores ultrapassa muito este valor
```{r}
names(contratos)
...
mediana <- median(data_fornecedores_groups$n)
