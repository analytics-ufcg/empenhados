...
Um fornecedor é aquele que fornece mercadorias ou serviços a um consumidor, nesta análise em especial, considera-se como fornecedor toda empresa ou pessoa física que está ligado a uma licitação que é considerada 100% de merenda. Ou seja, todos os empenhos dessa licitação são destinados a alimentação escolar e quaisquer empresas ou pessoas físicas que lançaram propostas para tais licitações serão consideradas fornecedores de merenda.
...
leaflet(data = mapa_paraiba) %>%
  ...
  addPolygons(opacity = 0.5, 
              ...
              popup = paste('Município: ', mapa_paraiba@data$Nome_Munic, '</br>Número de fornecedores: ', mapa_paraiba@data$total),
              fillOpacity = 1) %>%
...
Como dito anteriormente, vamos agora fazer um estudo mais criterioso dos casos em que um fornecedor ganha vezes que o número de vezes que participou, vale lembrar que esta inconsistência pode ter ocorrido por diversos fatores, como por exemplo erro na inserção do CPF ou CNPJ de um fornecedor.
...
ganhamais <- lucros %>% filter(participou < ganhou_l) %>%
  ...
  select(nu_CPFCNPJ, no_Fornecedor, ganhou, participou, perdeu, valor)
