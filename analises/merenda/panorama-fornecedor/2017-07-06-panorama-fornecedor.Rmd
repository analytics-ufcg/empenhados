...
lucros %>%
  ...
  summarise(freq = n()) %>%
    mutate(total = cut(freq, 
                     breaks = c(1, 25, 50, 100, 200, 500, 516),
                     include.lowest = T)) %>%
  ggplot(aes(x=participou, y=ganhou, size=total, color = total)) +
...
```{r}
oxe <- lucros %>% filter(participou > ganhou) %>%
  left_join(fornecedores, by = "nu_CPFCNPJ") %>%
  group_by(nu_CPFCNPJ) %>%
  summarise(n =)
