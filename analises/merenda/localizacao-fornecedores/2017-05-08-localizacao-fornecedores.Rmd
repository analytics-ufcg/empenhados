      ...
      inputPanel(
        selectInput(inputId = "busca", 
          ...
          radioButtons(inputId = "modo_visualizacao", 
                       label = "Verificar", 
                       choices = c("Quantidade de empenhos", "Valor pago"),
                       selected = "Quantidade de empenhos")
      )
    ),
...
server <- function(input, output, session){
  ...
  # mapa_paraiba@data <- data_pb %>%
  #     left_join(
  #       (
  #         localizacao_licitantes_municipios  %>%
  #         filter(grepl("00241811000184", cd_Credor)) %>%
  ...
  #                 weight = 1,
  #                 color = 'black',
  #                 fillColor = colorFactor('Blues', mapa_paraiba@data$ganhou)(mapa_paraiba@data$ganhou),
  #                 label = paste(mapa_paraiba@data$Nome_Munic),
  #                 popup = paste("Município: ", str_to_upper(mapa_paraiba@data$Nome_Munic), "</br>",
  #                               "Empenhos: ", mapa_paraiba@data$ganhou, "</br>",
  #                               "Valor pago: R$ ", mapa_paraiba@data$valor_total_pag),
  #                 fillOpacity = 1) %>%
  #      addMarkers(
  #        data = (
           # localizacao_licitantes_municipios %>%
           #    filter(grepl("00241811000184", str_sub(cd_Credor, -14))) %>%
           #    left_join(dados_cep.selected, by = c("nu_CEP" = "cep"))
  #        ),
  #        label = ~str_to_upper(no_Credor)
  #     )
