...
source("../lib/load_dados_escolares.R")
```
...
matriculas_municipios <- matriculas_municipios %>%
  rowwise() %>%
  mutate(ano2006 = approxfun(
      x = c(0,2,4,7,10), 
      y = c(ano2005, ano2007, ano2009, ano2012, ano2015)
      )(1)
    ) %>%
  mutate(ano2008 = approxfun(
      x = c(0,2,4,7,10), 
      y = c(ano2005, ano2007, ano2009, ano2012, ano2015)
      )(3)
    ) %>%
  mutate(ano2010 = approxfun(
      x = c(0,2,4,7,10), 
      y = c(ano2005, ano2007, ano2009, ano2012, ano2015)
      )(5)
    ) %>%
  mutate(ano2011 = approxfun(
      x = c(0,2,4,7,10), 
      y = c(ano2005, ano2007, ano2009, ano2012, ano2015)
      )(6)
    ) %>%
  mutate(ano2013 = approxfun(
      x = c(0,2,4,7,10), 
      y = c(ano2005, ano2007, ano2009, ano2012, ano2015)
      )(8)
    ) %>%
  mutate(ano2014 = approxfun(
      x = c(0,2,4,7,10), 
      y = c(ano2005, ano2007, ano2009, ano2012, ano2015)
      )(9)
    ) %>%
  mutate(ano2006 = as.integer(ano2006)) %>%
  mutate(ano2008 = as.integer(ano2008)) %>%
  mutate(ano2010 = as.integer(ano2010)) %>%
  mutate(ano2011 = as.integer(ano2011)) %>%
  mutate(ano2013 = as.integer(ano2013)) %>%
  mutate(ano2014 = as.integer(ano2014))

# escolas_municipios <- dados_educacao %>%
#   select(-vl_Matriculas_Total) %>%
#   spread(key = dt_Ano, value = vl_Escolas_Total) %>%
#   rename(ano2005 = `2005`) %>%
#   rename(ano2007 = `2007`) %>%
#   rename(ano2009 = `2009`) %>%
#   rename(ano2012= `2012`) %>%
...
dados_educacao <- load_dados_escolares() %>%
  mutate(vl_Matriculas_Total = vl_Matriculas_Fundamental + vl_Matriculas_Medio + vl_Matriculas_Pre_Escolar) %>%
  mutate(vl_Escolas_Total = vl_Escolas_Fundamental + vl_Escolas_Medio, vl_Matriculas_Pre_Escolar) %>%
...
Quanto será que São José do Brejo do Cruz gastou no total em outros anos?
...
plot_ly(data = gasto_sao_jose, x = ~as.factor(dt_Ano)) %>%
  add_trace(y = ~total_pago, type = 'scatter', name = 'Total', mode = 'lines+markers') %>%
  layout(title = "Total Pago em São José do Brejo do Cruz",
         yaxis = list(title = 'Total Pago (em milhares de reais)', hoverformat= '.2f'),
         xaxis = list(title = 'Ano'))
```

Como não temos os dados sobre o número de matriculados em todos os anos, não é possível comprarar o custo por aluno. No entanto é possível comparar quanto foi pago no total de empenhos relacionados a alimentação escolar. Apesar de em 2015, São José do Brejo do Cruz ser o município que mais gastou por aluno na Paraíba, esse não é o ano que o município mais gastou com merenda, na verdade é 2016, seguido de 2014. O ano de 2017 está bem abaixo dos demais pois os dados analisados aqui vão até março de 2017.
...
Como já dito anteriormente, Serra Branca gastou menos de 1 real por aluno em 2015 e no total gastou 849.97 reais. Ao compararmos com anos vizinhos como 2014 e 2016 esse gasto foi 10 vezes maior nesses anos. Em 2016 foram gastos 8015.20 reais e em 2016 foram gastos 9117.70 reais. Ainda sim, o ano de 2009 apresenta o maior valo pago pelo município em empenhos relacionados a alimentação escolar.
...
Compartilhe nos comentários sobre o que você achou sobre a análise, e quais experiências com merenda no seu município você sabe ou já presenciou.  
