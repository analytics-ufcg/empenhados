...
Sabendo algumas características sobre Boa Ventura nota-se que é uma cidade relativamente pequena, com número de matrículas totais de `r subset(dados_escolares, dados_escolares$cidade == 'Boa Ventura')$matritotal` alunos, inferior a média da Paraíba por município que é de `r round(mean(dados_escolares$matritotal), 2)` e a mediana de `r round(median(dados_escolares$matritotal), 2)` alunos.
...
Santa Inês com `r subset(municipios, municipios$de_Municipio == 'Santa Inês')$vl_Populacao` foi o município que mais gastou por aluno em 2015, seus dados escolares são:
...
De acordo com as histórias acima é possível dizer que o filtro utilizado até então, que captura todas as licitações que possuem merenda em sua descrição, fez sentido e obteve sucesso nas cidades que mais gastaram por aluno em 2015, uma vez que tais cidades geralmente realizaram apenas uma licitação de merenda para todo o ano. Em geral, as cidades que menos gastaram por aluno em 2015 não tiveram todas as suas licitações capturadas pelo filtro. 
...
Ou seja, a análise de custo por aluno é válida, no entanto o filtro que captura as licitações de merenda não consegue ser suficientemente preciso ao ponto de capturar a maioria das licitações de alimentação escolar. Portanto, algumas cidades tem seus índices de gasto por aluno alterados devido a essa deficiência no filtro.
