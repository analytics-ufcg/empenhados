...
Para verificar se isso realmente é uma observação relevante, faremos uma análise utilizando os empenhos que possuem todos seus pagamentos registrados no mesmo dia do empenho em questão e os dados de seus credores.
  ...
  mutate(
    ...
    vl_Empenho.group = cut(
      ...
      labels = c(
        ...
        '200M a 250M', '250M a 300M', '300M a 350M', '350M a 400M', '400M ou mais'),
      include.lowest = TRUE,
      ordered_result = TRUE))
```
...
#### Valor total gasto por município
...
Vemos que a distribuição do número de empenhos é bastante regular para os municípios, mas o mesmo não ocorre para o valor total destes. Grande parte das cidades totalizaram até 50 milhões de reais gastos com empenhos deste tipo, mas cidades populosas como Campina Grande e João Pessoa somaram mais de 400 milhões de reais. Um comportamento como este já era esperado, uma vez que cidades maiores tem um volume de gastos maior e grande parte das cidades da Paraíba são de pequeno porte.

### Quais são os credores mais relacionados a esses empenhos?

Abaixo estão representados os 10.000 credores que fizeram o maior número de empenhos com algum pagamento no mesmo dia. A cor de cada um deles indica qual o tipo de atividades que eles exercem de acordo com seus registros no CNAE. Note que ambos os eixos do gráfico estão em uma escala logarítmica, com o intuito de facilitar a visualização de valores muito altos.
