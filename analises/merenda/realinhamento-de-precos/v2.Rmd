...
query <- sql('
  ...
  FROM Empenhos e
  WHERE cd_Funcao = 12
  AND (cd_Subfuncao = 306 OR cd_Subelemento = "02")
')
...
empenhos.full <- empenhos.full %>%
  mutate_at(
    vars(vl_Estorno, vl_EstornoLiquidacao, vl_EstornoPagamento),
    funs(replace(., is.na(.), 0))
  )

empenhos.full <- empenhos.full %>%
  mutate_at(
    vars(vl_Empenho, vl_Liquidacao, vl_Pagamento),
    funs(replace(., .==0, NA))
  )
```

```{r}
empenhos.alt <- empenhos.full %>%
  mutate(
...
licitacoes.full <- licitacoes.full %>%
  mutate_at(
    vars(vl_Licitacao, vl_Contrato, vl_Empenho, vl_Liquidacao, vl_Pagamento),
    funs(replace(., .==0, NA))
  )
```
...
Assim como no caso acima, há muitos pontos sobre os eixos do gráfico. Esses ocorrem porque os empenhos em questão não estão relacionados a nenhum contrato ou vice versa.
...
g <- ggplot(aes(x=log(vl_Empenho+1), y=log(vl_Pagamento+1)), data = empenhos.alt) +
    geom_abline(slope=1, intercept=0, color='grey') +
    geom_point() +
    scale_x_continuous(breaks=pretty_breaks(10)) +
    scale_y_continuous(breaks=pretty_breaks(10)) +
