...
toclust = select(licitantes2.scaled.kmeans, 1:7)

row.names(toclust) = licitantes2.scaled$nuCPFCNPJ
dists = dist(toclust)

km <- kmeans(toclust, centers = 7, nstart = 10)

autoplot(km, data = licitantes2.scaled.kmeans, size = 2)
...
```{r}
plot(silhouette(km$cluster, dists), col=colors, border=NA)
```
