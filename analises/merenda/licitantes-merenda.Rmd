...
```{r}
licitantes2.scaled.kmeans = select(licitantes2, -nu_CPFCNPJ) %>% 
  mutate_each(funs(scale))

licitantes2.scaled.kmeans$nu_CPFCNPJ = licitantes2$nu_CPFCNPJ

set.seed(12346) # Garantir que o k-means gere o mesmo resultado
km <- kmeans(select(licitantes2.scaled.kmeans, 1:7), centers = 7, nstart = 10)

autoplot(km, data = licitantes2.scaled.kmeans, size = 2, main = "Grupos obtidos utilizando o k-means") + 
  labs(colour = "Grupos") 

licitantes2.scaled.kmeans$kmcluster = factor(km$cluster)
licitantes2.long.kmeans = melt(licitantes2.scaled.kmeans, id.vars = c("nu_CPFCNPJ", "kmcluster"))
```

``` {r}
ggplot(licitantes2.long.kmeans, aes(x = variable, y = value, colour = variable)) + 
  geom_boxplot() + 
  geom_point(alpha = 0.2, position = position_jitter(width = .2)) + 
...
p2 = ggplot(licitantes2.long.kmeans, aes(x = variable, y = value, group = nu_CPFCNPJ, colour = kmcluster)) + 
  geom_point(alpha = 0.2) + 
  geom_line(alpha = .3) + 
  facet_wrap(~ kmcluster, ncol = 2) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(x= "", y = "")
...
```{r}
plot(silhouette(km$cluster, dist(select(licitantes2.scaled.kmeans, 1:7))), col=colors, border=NA, main = "Silhouette dos grupos utilizando o k-means")
```
...
licitantes2.scaled.dbscan = licitantes2.scaled
licitantes2.scaled.dbscan$cluster = factor(dbs$cluster)

pr.out = prcomp(select(licitantes2.scaled.dbscan, - c(cluster, nu_CPFCNPJ)), scale = FALSE) 

autoplot(pr.out, data = licitantes2.scaled.dbscan, colour = 'cluster', scale = 0, main = "Clusters obtidos usando o algoritmo DBScan")
```
...
>>>>>>> f4f36a7d8dd63939a8bc2b50aef50a2ed7696196

#### Análise dos resultados
```{r}
licitantes2.long.dbscan = melt(licitantes2.scaled.dbscan, id.vars = c("nu_CPFCNPJ", "cluster"))

ggplot(licitantes2.long.dbscan, aes(x = variable, y = value, colour = variable)) + 
  geom_boxplot() + 
  geom_point(alpha = 0.2, position = position_jitter(width = .2)) + 
  facet_wrap(~ cluster) +
  theme(axis.text.x=element_blank(), 
        axis.ticks.x=element_blank()) +
  labs(title = "Agrupamento de licitantes de merenda da Paraíba", x = "", y = "", colour = "Variável")
```

```{r}
p2 = ggplot(licitantes2.long.dbscan, aes(x = variable, y = value, group = nu_CPFCNPJ, colour = cluster)) + 
  geom_point(alpha = 0.2) + 
  geom_line(alpha = .3) + 
  facet_wrap(~ cluster, ncol = 2) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
...
```{r}
licitantes.hcvariance = licitantes2 %>%
  select(-nu_CPFCNPJ) %>%
  mutate(cluster = licitantes2.scaled$cluster) %>%
  group_by(cluster) %>%
  summarise_each(funs(var)) %>%
  summarise_each(funs(mean)) %>%
  rename(metodo = cluster) %>%
  mutate(metodo = 'HC')

licitantes.kmvariance = licitantes2 %>%
  select(-nu_CPFCNPJ) %>%
  mutate(cluster = licitantes2.scaled.kmeans$kmcluster) %>%
  group_by(cluster) %>%
  summarise_each(funs(var)) %>%
  summarise_each(funs(mean))%>%
  rename(metodo = cluster) %>%
  mutate(metodo = 'Kmeans')

licitantes.dbsvariance = licitantes2 %>%
  select(-nu_CPFCNPJ) %>%
  mutate(cluster = licitantes2.scaled.dbscan$cluster) %>%
  filter(cluster != 1) %>%
  group_by(cluster) %>%
  summarise_each(funs(var)) %>%
  summarise_each(funs(mean)) %>%
  rename(metodo = cluster) %>%
  mutate(metodo = 'DBScan')

licitantes.mvariance = rbind(licitantes.hcvariance,
                            licitantes.kmvariance,
                            licitantes.dbsvariance)

kable(licitantes.mvariance, align = 'c')
```
