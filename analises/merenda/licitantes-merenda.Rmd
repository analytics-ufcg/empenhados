...
hullplot(licitantes2.scaled[1:7], dbs, col = colors)
```
...
autoplot(pr.out, label = TRUE, shape = FALSE, color = )

autoplot(pr.out, label = TRUE, label.size = 3, shape = FALSE, 
         loadings = TRUE, loadings.colour = 'blue',
         loadings.label = TRUE, loadings.label.size = 3)

ggplot(licitantes2.scaled[1:7], aes(color = cluster)) +
  geom_point()
...
```{r}
dbs2 = dbs[dbs$cluster == 0]
plot(silhouette(dbs2$cluster, dists), border = NA)
```
