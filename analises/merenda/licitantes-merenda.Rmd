...
```{r}
ggplot(licitantes.hc.long, aes(x = variable, y = value, colour = variable)) +
  geom_boxplot() +
...
```{r}
p1 <- ggplot(licitantes.hc.long, aes(x = variable, y = value, group = nu_CPFCNPJ, colour = cluster)) +
...
```{r}
p2 <- ggplot(licitantes.km.long, aes(x = variable, y = value, group = nu_CPFCNPJ, colour = cluster)) +
  geom_point(alpha = 0.2) +
  geom_line(alpha = .3) +
  facet_wrap(~ cluster, ncol = 2) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
```{r}
ggplot(licitantes.dbs.long, aes(x = variable, y = value, colour = variable)) +
  geom_boxplot() +
  geom_point(alpha = 0.2, position = position_jitter(width = .2)) +
  facet_wrap(~ cluster) +
  theme(axis.text.x=element_blank(),
...
```{r}
p3 <- ggplot(licitantes.dbs.long, aes(x = variable, y = value, group = nu_CPFCNPJ, colour = cluster)) +
  geom_point(alpha = 0.2) +
  geom_line(alpha = .3) +
  facet_wrap(~ cluster, ncol = 2) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
```{r}
hc.var <- licitantes.log %>%
  select(-nu_CPFCNPJ) %>%
  mutate(cluster = licitantes.hc$cluster) %>%
  group_by(cluster) %>%
...
km.var <- licitantes.log %>%
  select(-nu_CPFCNPJ) %>%
  mutate(cluster = licitantes.km$cluster) %>%
  group_by(cluster) %>%
  summarise_each(funs(var)) %>%
  select(-cluster) %>%
  summarise_each(funs(mean)) %>%
...
dbs.var <- licitantes.log %>%
  select(-nu_CPFCNPJ) %>%
  mutate(cluster = licitantes.dbs$cluster) %>%
  filter(cluster != 1) %>%
  group_by(cluster) %>%
  summarise_each(funs(var)) %>%
  select(-cluster) %>%
...
m.var <- rbind(hc.var, km.var, dbs.var) %>%
  select(metodo, participou:aditivos)
