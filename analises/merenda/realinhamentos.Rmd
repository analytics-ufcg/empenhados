...
O gasto do dinheiro público é, para a maioria dos cidadãos, um processo confuso e complexo, no entanto compreender como se desenrrola este método pode ser fundamental tanto para termos noção de como o nosso dinheiro é gasto quanto para identificarmos peculiaridades no gasto desse dinheiro.
...
Outra relação importante é a existente entre o empenho e a liquidação de uma licitação, apresentada no gráfico seguinte.
...
Esta relação apresenta mais dados de acordo com o esperado do que a relação estudada anteriormente (empenhos vs. contratos) uma vez que existem muito mais valores exatamente sobre a reta traçada que identifica os casos em que o valor do empenho é igual ao valor da liquidação.
Esistem ainda assim valores que não estão sobre a reta, quer seja abaixo dela, implicando que o valor empenhado foi maior que o liquidado, quer seja acima significando exatamente o oposto; no entanto existem poucas observações com este comportamento e as que assim se apresentam estão próximas à reta, pessupondo que as diferenças entre empenho e liquidação são baixas.
...
Verificamos aqui que os valores de liquidação e pagamento são, na maioria das vezes, iguais. Neste caso, ao contrário do anterior, é muito mais comum que o valor do pagamento seja menor que o da liquidação do que o inverso.
Com isso compreeende-se que na grande maioria das vezes está sendo liquidado exatamente o necessário para realizar o pagamento ao fornecedor e quando isso não é verdade o que ocorre é que é liquidado um valor maior que o que é verdadeiramente pago.
