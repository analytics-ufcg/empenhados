...
```{r}
kable(
  x = head(arrange(select(municipios, c(de_Municipio, gasto_por_aluno, faixa_pop)), desc(gasto_por_aluno)), 5),
...
```{r}
kable(
  x = head(arrange(select(municipios, c(de_Municipio, gasto_por_aluno, faixa_pop)), gasto_por_aluno), 10),
  col.names = c('Município', 'Gasto por aluno', 'Faixa de população'),
  align = c('c'))
```
