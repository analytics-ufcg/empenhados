...
Percebemos no gráfico que alguns municípios, considerados outliers podem estar afetando a correlação linear. A seguir iremos olhar uma nova visualização e correlação desconsiderando esses outliers.

```{r}
municipios.alt <- municipios %>%
         filter(!de_Municipio %in% c("São Francisco", "São Sebastião do Umbuzeiro", "Bernardino Batista", "São Domingos", "São José do Brejo do Cruz", "Curral Velho", "Cacimba de Areia", "Montadas", "Bom Sucesso", "São Vicente do Seridó", "Livramento", "Belém do Brejo do Cruz", "Aparecida", "Condado", "São João do Rio do Peixe", "Fagundes", "Serra Branca", "Riacho dos Cavalos"))


ggplot(data = municipios.alt, 
       aes(x = gasto_por_aluno, y = IDEB_medio_2015, color = faixa_pop)) +
  geom_point() + 
  geom_smooth() +
  facet_wrap(~ faixa_pop, ncol = 2)
```

Observamos que para algumas faixas de população, como [0, 4K], parece haver uma correlação linear entre as variáveis. A linha ajuda a entender como se dá a distribuição dos pontos e busca representar a forma dessa distribuição. A seguir iremos verificar em termos estatísticos qual a correlação linear entre as variáveis para essas cidades. Lembrando que tanto na visualização acima como na tabela abaixo ignoramos alguns municípios considerados outliers.

```{r}
municipios_group_alt <- municipios.alt %>%
  group_by(faixa_pop) %>%
  summarise(n = n(),
            gasto_medio = round(mean(gasto_por_aluno), 2),
            gasto_mediano = round(median(gasto_por_aluno), 2),
...
kable(
  x = head(select(municipios_group_alt, c(faixa_pop, n, gastoXideb, gastoXpop, gasto_mediano))),
  col.names = c('Faixa da população', 'Nº de municípios', 'Gasto por aluno X Ideb', 'Gasto por aluno X População', "Gasto mediano"),
  align = c('c'))
```

Voltando a considerar todos os municípios, o gasto por aluno parece apresentar casos com comportamento _estranho_, uma vez que, na mediana os municípios da paraíba gastaram `r round(median(municipios$gasto_por_aluno), 2)` reais com a alimentação de cada aluno no ano de 2015, no entanto alguns municípios parecem ter gasto muito mais que isso. Alguns deles são:
...
### Conclusão
Com base no que foi estudado, mesmo lançando mão de diferentes critérios para a análise, concluimos que o gasto com merenda e o Ideb de um município não estão diretamento relacionados.  
