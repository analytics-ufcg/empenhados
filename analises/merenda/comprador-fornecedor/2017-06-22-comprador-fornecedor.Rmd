...
paraiba_sumarizado$total = cut(paraiba_sumarizado$total,
                                breaks = c(0,5,7,10,15,25,50, 120),
...
paraiba_sumarizado$valor_medio_emp <- cut(paraiba_sumarizado$valor_medio_emp,
                                         breaks = c(0, 15000, 30000, 50000, 75000, 100000, 200000, 100000000),
                                         labels = c("De 0 a 15", "De 15 a 30", "De 30 a 50", "De 50 a 75", "De 75 a 100", "De 100 a 200", "Acima de 200"),
                                          include.lowest = F)

paraiba_sumarizado$valor_total_emp <- cut(paraiba_sumarizado$valor_total_emp,
                                         breaks = c(0, 250000, 500000, 750000, 1000000, 2000000, 4000000,1000000000),
                                         labels = c("De 0 a 250", "De 250 a 500", "De 500 a 750", "De 750 a 1000", "De 1000 a 2000", "De 2000 a 4000", "Acima de 4000"))
# max(paraiba_sumarizado$valor_total_emp_num)
# ggplot(data = paraiba_sumarizado %>% filter(valor_total_emp_num < 5000000) , aes(x = valor_total_emp_num)) + 
...
leaflet(data = mapa_paraiba) %>%
  ...
  addPolygons(opacity = 0.5,
              ...
              color = 'black',
              label = paste(mapa_paraiba@data$Nome_Munic, "Número de licitações:", mapa_paraiba$X),
              fillOpacity = 1) %>%
...
leaflet(data = mapa_paraiba) %>%
  ...
  addPolygons(opacity = 0.5,
              ...
              fillOpacity = 1) %>%
  addLegend(position = "bottomright", pal = colors, values = mapa_paraiba@data$Y,
            title = "Valor médio dos empenhos de merenda",
            opacity = 1
  )
```
...
```{r}
kable(
  x = head(arrange(select(paraiba_sumarizado, c(de_Municipio, valor_medio_emp_num)), desc(valor_medio_emp_num)), 10),
  col.names = c('Município', 'Valor médio dos empenhos de merenda'),
  align = c('c'))
```
...
leaflet(data = mapa_paraiba) %>%
  ...
  addPolygons(opacity = 0.5,
              ...
              fillOpacity = 1) %>%
  addLegend(position = "bottomright", pal = colors, values = mapa_paraiba@data$Y,
            title = "Valor total dos empenhos de merenda",
            opacity = 1
  )
```
