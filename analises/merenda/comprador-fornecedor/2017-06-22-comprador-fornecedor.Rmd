...
### Qual o comportamento da média e da mediana do valor das licitações de merenda ao longo dos anos (2011-2015)?
...
### Em geral, as licitaçoes mais caras estão concentradas em alguma época do ano?
...
### Quantas propostas, em geral, recebem as licitações de merenda?
...
É possível perceber que em geral as licitações recebem poucas prospostas, na verdade, `r round(nrow(filter(licitacoes_merenda, nu_Propostas <= 3))/nrow(licitacoes_merenda) * 100, 2)`% das licitações tem no máximo 3 propostas.  
...
```
Observando a visualização observamos que aparentemente não temos correlação linear entre as variáveis. O que pode ser confirmado pelo valor de correlação linear entre elas que é de `r cor1`.  
Mas nesse gráfico temos alguns valores extremos que nos impedem de observar com mais detalhes onde nossos dados estão realmente concentrados, em função disso vamos desconsiderar esses valores extremos. gerar um novo gráfico e um novo valor de correlação linear entre variáveis como segue.
...
Com a aproximação temos um valor de correlação linear ainda menor (`r cor2`), o que nos concluir que não existe correlação linear entre o valor da licitação e o número de participantes da mesma.