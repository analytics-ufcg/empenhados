...
Podemos perceber que houve um salto no total de licitações entre 2012 e 2013, o que pode indicar que os municípios passaram fazer mais licitações envolvendo merenda. Esse salto acontece também nos dados gerais de licitação e já foi analisado neste mesmo relatório. Aqui, também chama a atenção o ano de 2015, que apresenta o maior número de licitações desse tipo.
...
```{r}
kable(
  x = head(arrange(select(paraiba_sumarizado, c(de_Municipio, total_num, total_emp)), desc(total_num)), 5),
  col.names = c('Município', 'Número de Licitações', 'Número de Empenhos'),
  align = c('c'))
```
...
A partir do gráfico acima podemos observar que as modalidades mais utilizadas nas licitações de merenda atualmente são o pregão, eletrônico ou presencial, e a chamada pública. Até o ano 2013, a modalidade convite era bastante utilizada para esse tipo de licitação, mas seu uso caiu bastante desde então. Atualmente o convite não está sequer entre as cinco modalidades mais utilizadas.
...
O valor de r, destacado no gráfico, indica o grau de correlação direta entre os dois fatores observados e deve variar entre -1 e 1. Quanto mais próximo de 0 é esse valor, menor é a possibilidade de haver correlação. 

Em nosso caso, o valor obtido para r foi de `r cor2` e indica uma possibilidade quase inexistente de correlação direta entre os valores. Em outras palavras, não temos sinais de que o valor de uma licitação influencie na quantidade de propostas recebidas pela mesma.  
