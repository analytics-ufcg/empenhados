...
licitacoes_merenda = licitacoes %>%
  filter(grepl('merenda', licitacoes$de_Obs, ignore.case = T))
```
...
metricas <- municipios %>%
  ...
  rename(
    ...
    `Licitacoes` = nu_Licitacoes,
    `Mediana_propostas` = nu_Propostas,
    `Gasto_total` = vl_Total,
    `Gasto_mediano` = vl_Medio,
    `Populacao` = vl_Populacao) %>%
  select(cd_IBGE, Fornecedores, Licitacoes, Mediana_propostas, Gasto_total, Gasto_mediano, Populacao)
...
```{r}
metricas <- log(select(metricas, -cd_IBGE))
...
```{r}
metricas %>%
  gather(metrica, valor, -Populacao) %>%
  ggplot(aes(x=Populacao, y=valor)) +
    geom_point(na.rm=T) +
