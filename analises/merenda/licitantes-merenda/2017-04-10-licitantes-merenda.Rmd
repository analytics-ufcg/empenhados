...
dists <- dist(select(licitantes.scaled, -c(nu_CPFCNPJ, no_Fornecedor)), method = 'euclidean')
...
licitantes.hc.long <- licitantes.hc %>%
  gather('variable', 'value', -nu_CPFCNPJ, -no_Fornecedor, -cluster, factor_key=T)
```

```{r}
pr.out <- prcomp(select(licitantes.scaled, -c(nu_CPFCNPJ, no_Fornecedor)), scale = FALSE)
