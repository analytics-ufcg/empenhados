...
```{r}
contratos <- read_csv("../../../utils/dados/transporte_escolar/contratos.csv")
empenhos <- read_csv("../../../utils/dados/transporte_escolar/empenhos.csv")
licitacoes <- read_csv("../../../utils/dados/transporte_escolar/licitacoes.csv")
fornecedores <- read_csv("../../../utils/dados/transporte_escolar/fornecedores.csv")
unidades_gestoras <- read_csv("../../../utils/dados/transporte_escolar/unidades_gestoras.csv")
pagamentos <- read_csv("../../../utils/dados/transporte_escolar/pagamentos.csv")
educacao.municipios <- read_csv("../../../utils/dados/transporte_escolar/dados_educacao_municipios.csv")
licitacoes_total <- read_csv("../../../utils/dados/transporte_escolar/licitacoes_total.csv")
```

Nesse post realizaremos uma análise sobre os gastos com transporte escolar no estado da Paraíba em 2016. Os dados utilizados foram obtidos a partir da plataforma SAGRES do Tribunal de Contas do Estado da Paraíba, que contém informações sobre gastos públicos dos municípios.
...
No gráfico acima, vemos que o mês de maio foi o que teve maior número de licitações de transporte escolar, seguido de junho e julho, respectivamente. Isso chama atenção, pois o esperado é que a maioria das licitações ocorram nos primeiros meses, quando são firmados os contratos do ano inteiro. 
...
licitacoes_municipios %>%
  ...
  theme_ipsum()
```
Como vemos acima, dentre os municípios que mais realizaram licitações, os que mais se destacaram foram Igaracy e Água Branca.
...
Observando o gráfico acima, vemos que a modalidade mais comum nas licitações sobre transporte escolar é o **Pregão Presencial**. Nas versões presencial e eletrônica, o pregão não determina limites para os valores, sendo que a disputa ocorre durante sessão pública. O vencedor é o concorrente que oferece o menor preço.
...
Vemos que licitações de valores mais altos receberam mais propostas. Embora existam licitações de valor alto que receberam poucas propostas, não é possível ver no gráfico, por exemplo, licitações que receberam mais que 10 propostas com valor menor que R\$ 100 mil. 
...
licitacoes.mediana %>% 
  ...
  datatable(
    rownames = FALSE,
    class = "cell-border stripe",
    colnames = c("Modalidade", "Valor (Mediana)")
  ) %>%
...
No gráfico acima, a linha preta representa a mediana, e o tamanho do ponto a quantidade de licitações daquela modalidade com aquele valor. Com exceção das modalidades que possuem apenas uma observação, vemos que os valores das licitações não se concentram em uma mesma faixa. Vejamos mais detalhes sobre as licitações de valor mais alto.
...
licitacoes.caras %>%
  ...
  datatable(
    ...
    rownames = FALSE,
    class = "cell-border stripe",
    colnames = c("Licitação", "Descrição", "Valor", "Município")
  ) %>%
...
## Municípios que mais empenharam 
...
A média geral do valor empenhado para contratos, em 2016, foi de R\$ `r media.empenhos` e a mediana foi de R\$ `r mediana.empenhos`. Não há uma distribuição igualitária entre os valores empenhados pelos municípios do estado; alguns municípios gastam muito mais que os outros. Monteiro e Sousa foram os municípios que empenharam mais dinheiro em comparação aos demais, pois a soma dos valores empenhados nesses ultrapassa R\$ 1 mi.

## Quais os fornecedores que receberam mais destes empenhos?
...
A partir do que foi empenhado, procuramos os municípios que mais gastaram em pagamentos para estes serviços. Estamos considerando o valor gasto por município como sendo a soma dos valores de pagamentos referentes às licitações realizadas.
...
No gráfico acima temos os dez municípios que mais gastaram em pagamentos relacionados a contratos de transporte escolar. O município de Sousa é um dos municípios que mais gastou, o que faz sentido considerando que este município também está entre os que mais empenharam e mais licitaram em 2016. Analisando o número da população dos municípios listados acima, vemos que existe uma grande diferença entre eles. Isso nos leva a suspeitar que a população de uma cidade não influencia nos gastos com transporte escolar.
...
Vemos que a maioria das cidades gastam no máximo até R$ 50,00 por aluno com transporte escolar durante um ano. Observamos, também, que poucos municípios gastam mais que R\$ 100. Quais são esses municípios?
