...
```{r}
n <- nrow(mapa_paraiba@data)
mapa_paraiba@data$Y <- rnorm(n)
mapa_paraiba@data$X <- factor(sample(LETTERS[1:4], size = n, replace = T))
spplot(mapa_paraiba, "Y")
```

```{r}
municipios$de_Municipio = factor(municipios$de_Municipio)
paraiba = mapa_paraiba@data %>% inner_join(municipios, c('Nome_Munic' = 'de_Municipio'))
```
