---
title: "Análise das Licitações de Merenda na Paraíba - Parte 1"
...
# Conclusão
Ao analisar os dados das licitações de merenda na Paraíba, pelo lado do comprador, pudemos observar que a modalidade mais comum de licitação é a Convite, e se gasta em média por licitação `r format(mean(licitacoes_merenda$vl_Licitacao), scientific=FALSE)` reais. Os gastos com merenda são maiores no começo do ano, período em que ocorre também a maior quantidade de licitações dessa natureza. Os municípios que mais lançaram licitações de merenda no período estudado foram Santa Rita e Monteiro.  
Durante a análise algumas discrepâncias foram observadas, dentre elas o fato de João Pessoa ter aberto apenas 3 licitações de merenda durante todo o período analisado. Há também algumas licitações de merenda com valores muito mais altos se comparadas a outras licitações.  
Por fim espera-se que essa análise tenha sido de grande utilidade para que o contexto de licitação de merenda na Paraíba seja melhor compreendido pelos cidadãos do Estado.  
