...
Foram analisados dados no período compreendido entre os anos 2011 e 2015 e observou-se que o intervalo entre os meses de fevereiro e abril concentra a maioria das licitações de merenda e que o número de licitações desse tipo decai conforme nos aproximamos dos meses finais do ano.  
...
ggplot(aes(x = nu_Propostas), data = licitacoes_merenda) +
  geom_histogram(fill = I('#b2628b'), binwidth = 5) +
  labs(title = 'Total de propostas para licitações de merenda',
       x = 'Número de propostas', y = 'Total') +
  scale_x_continuous(breaks = seq(0, 50, 5)) +
  scale_y_continuous(breaks = seq(0,1500,250))
...
```{r}
ggplot(aes(x = nu_Propostas), data = licitacoes_merenda) +
  geom_histogram(fill = I('#6279b2'), binwidth = 1) +
  labs(title = 'Total de propostas para licitações de merenda',
       x = 'Número de propostas', y = 'Total') +
