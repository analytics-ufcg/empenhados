...
# Análises
## 4 - Em que período do ano ocorrem mais licitações de merenda? {#perg-4}
...
## 7 - Geralmente, quantas propostas são recebidas para licitações de merenda? {#perg-7}
...
## 8 - Que tipo de licitação é mais comum na compra de merenda? {#perg-8}
...
## 10 - A popularidade de alguma modalidade de licitação mudou ao longo dos anos? {#perg-10}
Dando prosseguimento à análise, foi verificado quais modalidades de licitação foram mais utilizadas para a obtenção de merenda entre os anos 2011 e 2015. Os resultados podem ser observados na figura lorem.  

```{r fig.cap ='Figura lorem'}
## Deve juntar os dois tipos de pregão?
## Por que a chamada pública só começa em 2013?
...
Ainda há outros pontos importantes a serem ressaltados nesse tópico. É possível observar, por exemplo, que há uma grande diferença entre o total de licitações dispensadas em 2013 e o de seus anos vizinhos e que a modalidade chamada pública começou a ser utilizada apenas nesse referido ano.    

## 11 - Em geral, as licitaçoes mais caras estao concentradas em alguma época do ano? {#perg-11}
...
## 12 - O valor da licitação está relacionado com o total de propostas recebidas? {#perg-12}
```{r fig.cap='Figura lorem'}
cor1 = round(with(licitacoes_merenda, cor(vl_Licitacao, nu_Propostas)), 2)
...
cor2 = round(with(licitacoes_merenda, cor(vl_Licitacao, nu_Propostas)), 2)
